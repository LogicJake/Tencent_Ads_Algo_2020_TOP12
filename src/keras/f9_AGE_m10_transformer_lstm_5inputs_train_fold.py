...
#####################################################################################
# AGE model 10: Keras transformer+LSTM 4 inputs 
# score: 
# 五折: 0.50795 (线下)
# 五折: 0.51653 (线上)
# 训练时长: ~ 3.5 days
...
checkpoint = callbacks.ModelCheckpoint(f'../../models/age_m10_{fold}.h5',
...
model.load_weights(f'../../models/age_m10_{fold}.h5')
preds = model.predict([test_cid, test_advid, test_aid, test_pid],
                      batch_size=batch_size,
                      verbose=1)

np.save(f'../../probs/sub_age_m10_keras_{fold}', preds)
...
np.save(f'../../probs/oof_age_m10_keras_{fold}', valid_preds)
