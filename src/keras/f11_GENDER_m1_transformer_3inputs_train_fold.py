...
#####################################################################################
# GENDER model 1: Keras transformer 3 inputs 
# score: 
# 五折: 0.94897 (线下)
# 五折: 0.95002 (线上)
# 训练时长: ~ 2.5 days
...
checkpoint = callbacks.ModelCheckpoint(f'../../models/gender_m1_{fold}.h5',
...
model.load_weights(f'../../models/gender_m1_{fold}.h5')
preds = model.predict([test_cid, test_advid, test_aid],
                      batch_size=batch_size,
                      verbose=1)

np.save(f'../../probs/sub_gender_m1_keras_{fold}', preds)
...
np.save(f'../../probs/oof_gender_m1_keras_{fold}', valid_preds)
