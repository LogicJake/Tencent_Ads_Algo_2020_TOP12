...
#####################################################################################
# AGE model 8: Keras transformer+LSTM 3 inputs (分开两路 advid+adid, cid, 最后用 Add)
# score: 
# 五折: 0.50496 (线下)
# 训练时长: ~ 3 days
#####################################################################################
...
def build_model(emb_cid, emb_advid, emb_aid):
    ...
    id_adv_ad_lstm = layers.Bidirectional(layers.LSTM(200, return_sequences=True))(mha1_out)
    ...
    id_c_lstm = layers.Bidirectional(layers.LSTM(200, return_sequences=True))(mha2_out)
...
checkpoint = callbacks.ModelCheckpoint(f'../../models/age_m8_{fold}.h5',
...
model.load_weights(f'../../models/age_m8_{fold}.h5')
preds = model.predict([test_cid, test_advid, test_aid],
                      batch_size=batch_size,
                      verbose=1)

np.save(f'../../probs/sub_age_m8_keras_{fold}', preds)
...
np.save(f'./probs/oof_age_m8_keras_{fold}', valid_preds)
