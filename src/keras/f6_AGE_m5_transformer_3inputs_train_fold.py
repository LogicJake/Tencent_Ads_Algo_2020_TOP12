...
#################################################################################
# AGE model 6: Keras transformer 3 inputs
# score: 
# 五折: 0.50605 (线下)
# 五折: 0.51527 (线上)
# 训练时长: ~ 2.5 days
...
checkpoint = callbacks.ModelCheckpoint(f'../../models/age_m5_{fold}.h5',
...
model.load_weights(f'../../models/age_m5_{fold}.h5')
preds = model.predict([test_cid, test_advid, test_aid],
                      batch_size=batch_size,
                      verbose=1)

np.save(f'../../probs/sub_age_m5_keras_{fold}', preds)
...
np.save(f'../../probs/oof_age_m5_keras_{fold}', valid_preds)
