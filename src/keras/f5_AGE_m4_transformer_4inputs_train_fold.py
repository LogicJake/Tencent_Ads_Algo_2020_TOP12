...
#################################################################################
# AGE model 4: Keras transformer 4 inputs
# score: 
# 五折: 0.50510 (线下)
# 五折: 0.51580 (线上)
# 训练时长: ~ 3 days
...
checkpoint = callbacks.ModelCheckpoint(f'../../models/age_m4_{fold}.h5',
...
model.load_weights(f'../../models/age_m4_{fold}.h5')
preds = model.predict([test_cid, test_advid, test_aid, test_pid],
                      batch_size=batch_size,
                      verbose=1)

np.save(f'../../probs/sub_age_m4_keras_{fold}', preds)
...
np.save(f'../../probs/oof_age_m4_keras_{fold}', valid_preds)
