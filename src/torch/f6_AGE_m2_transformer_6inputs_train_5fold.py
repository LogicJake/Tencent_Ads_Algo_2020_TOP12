...
#################################################################################
# AGE model 2: Torch Transformer+LSTM 6 inputs
# score: 
# 五折: 0.50299 (线下)
# 五折: 0.51589 (线上)
# 训练时长: ~ 5 days
...
from m2_transformer_6inputs_age import *
...
for fold_id, (trn_idx, val_idx) in enumerate(kfold.split(train_model_inputs)):
    ...
    early_stopping = EarlyStopping(
        file_name='../../models/age_m2_checkpoint{}.pt'.format(fold_id),
        patience=10,
        verbose=True,
        delta=0.00000001)
...
for fold_id, (trn_idx, val_idx) in enumerate(kfold.split(train_model_inputs)):
    ...
    model = LSTMCLF(seq_embedding_features=seq_embedding_features,
                    ...
                    seq_len=seq_len,
                    device=device).to(device)
    model.load_state_dict(torch.load('../../models/age_m2_checkpoint{}.pt'.format(fold_id)), strict=False)
...
np.save('../../probs/sub_age_m2_torch', test_pred_age)
np.save('../../probs/oof_age_m2_torch', oof_pred_age)
