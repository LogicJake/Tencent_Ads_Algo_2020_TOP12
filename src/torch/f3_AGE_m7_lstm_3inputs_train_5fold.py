#################################################################################
# AGE model 7: Torch LSTM+Attention 3 inputs
# score: 
# 五折: 0.50083 (线下)
# 五折: 0.51446 (线上)
# 训练时长: ~ 4 days
...
from m7_lstm_3inputs_age import *
...
print(train_model_inputs.shape, test_model_inputs.shape)
...
for fold_id, (trn_idx, val_idx) in enumerate(kfold.split(train_model_inputs)):
    ...
    optimizer = optim.Ranger(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-2)
    ...
    early_stopping = EarlyStopping(
        file_name='../../models/age_m7_checkpoint{}.pt'.format(fold_id),
        patience=10,
        verbose=True,
        delta=0.00000001)
...
for fold_id, (trn_idx, val_idx) in enumerate(kfold.split(train_model_inputs)):
    ...
    model = LSTMCLF(seq_embedding_features=seq_embedding_features,
                    ...
                    seq_len=seq_len,
                    device=device).to(device)
    model.load_state_dict(torch.load('../../models/age_m7_checkpoint{}.pt'.format(fold_id)), strict=False)    
    model.eval()
...
np.save('../../probs/sub_age_m7_torch', test_pred_age)
np.save('../../probs/oof_age_m7_torch', oof_pred_age)
